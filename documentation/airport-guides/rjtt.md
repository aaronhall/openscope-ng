...
There are four radar positions in the Tokyo Approach Control Area (ACA) — Depature, Approach, Radar, and TCA, with each being divided into smaller sectors. The Tokyo ACA covers both Tokyo Int'l (RJTT) and Narita Int'l (RJAA); however, due to the nature of openScope NG, aircraft will only be arriving and departing from Tokyo Int'l.
...
Note: Due to technical limitations in openScope NG, BRUCE transition of the ROVER departures are named AGRIS transitions instead.
