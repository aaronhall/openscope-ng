...
There are four radar positions in the Tokyo Approach Control Area (ACA) — Depature, Approach, Radar, and TCA, with each being divided into smaller sectors. The Tokyo ACA covers both Tokyo Int'l (RJTT) and Narita Int'l (RJAA); however, due to the nature of openScope NG, aircraft will only be arriving and departing from Narita Int'l.
...
The openScope NG scenario is set for the south wind operation, where departure is from 16L/R, but for most SIDs departing from 34L/R, there is an altitude restriction of 7000ft. This is to prevent conflicts between aircraft arriving at Tokyo Int'l. There will be no such traffic in openScope NG, so it is safe to immediately allow the aircraft to climb.
...
All the aircrafts arriving in openScope NG will head to NORMA (IAF for ILS Y/Z RWY 16L/R) following their assigned STAR. It is your responsibility to vector the aircraft to the final approach course.
