...
test('.applyArrivalProcedure() returns a success message after success', () => {
    ...
    expect(
        result[1].log === 'cleared to McCarran International Airport via the KEPEC3 arrival'
    ).toBe(true);
    expect(
        result[1].say === 'cleared to McCarran International Airport via the KEPEC THREE arrival'
    ).toBe(true);
});
...
test('.applyDepartureProcedure() returns a success message after success', () => {
    ...
    expect(_isArray(result)).toBe(true);
    expect(result[0]).toBe(true);
    expect(result[1].log === 'cleared to destination via the COWBY6 departure, then as filed').toBe(
        true
    );
    expect(
        result[1].say === 'cleared to destination via the COWBOY SIX departure, then as filed'
    ).toBe(true);
});
...
test.todo(
    '.replaceFlightPlanWithNewRoute() replaces old route with new one, and skips ahead to the old current waypoint'
);
...
test('.climbViaSID() correctly configures MCP and returns correct response when no altitude is given', () => {
    ...
    expect(pilot._mcp.altitude === pilot._fms.departureAirportModel.maxAssignableAltitude).toBe(
        true
    );
});
...
test('.conductInstrumentApproach() returns failure message when no runway is provided', () => {
    ...
    const result = aircraftModel.pilot.conductInstrumentApproach(
        aircraftModel,
        approachTypeMock,
        null
    );
...
test('.conductInstrumentApproach() returns failure message when assigned altitude is lower than minimum glideslope intercept altitude', () => {
    const expectedResult = [
        ...
        {
            log:
                'unable ILS 19L, our assigned altitude is below the minimum glideslope ' +
                'intercept altitude, request climb to 3700',
            say:
                'unable ILS one niner left, our assigned altitude is below the minimum ' +
...
test('.conductInstrumentApproach() calls ._interceptCourse() with the correct properties', () => {
    ...
    expect(
        _interceptCourseSpy.calledWithExactly(runwayModelMock.positionModel, runwayModelMock.angle)
    ).toBe(true);
});
...
test('.conductInstrumentApproach() calls ._interceptGlidepath() with the correct properties', () => {
    ...
    expect(
        _interceptGlidepathSpy.calledWithExactly(
...
test('.maintainAltitude() calls .shouldExpediteAltitudeChange() when shouldExpedite is true', () => {
    ...
    const shouldUseSoftCeilingMock = false;
    const shouldExpediteAltitudeChangeSpy = sinon.spy(
        aircraftModel.pilot,
...
test('.maintainAltitude() returns the correct response strings when shouldExpedite is true', () => {
    ...
    expect(result[1].say === 'descend and maintain flight level one niner zero and expedite').toBe(
        true
    );
});
...
test('.maintainHeading() returns a success message when incremental is false and direction is provided', () => {
    ...
    const aircraftModel = new AircraftModel(ARRIVAL_AIRCRAFT_INIT_PROPS_MOCK);
    const result = aircraftModel.pilot.maintainHeading(
        aircraftModel,
        42,
        directionMock,
        isIncremental
