...
const AIRPORT_POSITION_FIXTURE = new StaticPositionModel(
    ['N36.080056', 'W115.15225', '2181ft'],
    null,
    11
);
...
test('.calculateDistanceToPointForX() returns the distance from a reference position to a longitude', () => {
    ...
    const result = calculateDistanceToPointForX(
        AIRPORT_POSITION_FIXTURE,
        AIRPORT_POSITION_FIXTURE.latitude,
        longitudeMock
    );
...
test('.calculateDistanceToPointForY() returns the distance from a reference position to a latitude', () => {
    ...
    const result = calculateDistanceToPointForY(
        AIRPORT_POSITION_FIXTURE,
        latitudeMock,
        AIRPORT_POSITION_FIXTURE.longitude
    );
